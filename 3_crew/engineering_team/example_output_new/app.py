...
def get_account_info():
    ...
    if not report["holdings"]:
        ...
        for symbol, quantity in report["holdings"].items():
            ...
            value = price * quantity
            result += (
                f"{symbol}: {quantity} shares at ${price:.2f} each = ${value:.2f}\n"
            )
...
with gr.Blocks(title="Trading Simulation Platform") as demo:
    ...
    deposit_btn.click(
        fn=deposit, inputs=[deposit_input], outputs=[result_output, account_info]
    )

    withdraw_btn.click(
        fn=withdraw, inputs=[withdraw_input], outputs=[result_output, account_info]
    )
    ...
    transactions_btn.click(
        fn=get_transactions, inputs=[], outputs=[transactions_output]
    )
